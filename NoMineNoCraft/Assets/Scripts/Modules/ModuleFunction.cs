...
public class ModuleFunction : Module
{
    ...
    //The opposite
    public void UnlinkFromOutput(Module module, int inputNb = 0, int outputNb = 0)
    {
        foreach ((Module linkedModule, int linkedModuleInputNb) input in inputs[inputNb]){
            input.linkedModule.UnlinkFromOutput(module, input.linkedModuleInputNb, outputNb);
        }
    }
    ...
    {
        outputs[outputNb].linkedModule.RemoveListener(outputs[outputNb].linkedModuleOutputNb, func);
    }
